...
def GetTopLevelObject(obj = None):
	...
	while o != None:
		...
		try:
			...
			printDebug ("getTopLevel pas de parent")
			o = None
	GB_inGetTopLevelFunction -=1
	return oTop
...
def SayValue(value):
	...
	if len(value):	
		...
		voiceControl.updateSettings(api.getForegroundObject(), value)
		ui.message(value)
...
class AppModule(appModuleHandler.AppModule): 
	...
	def chooseNVDAObjectOverlayClasses(self, obj, clsList):
		...
		elif u"Aide-Mémoire" in name and "Menu" not in name:
			clsList.insert(0, InAideMemoireWindow)
			cls = "InAideMemoireWindow"
			
		elif u"Page" in name[:10] :
			clsList.insert(0, InPageWindow)
			cls = "InPageWindow"
			
		elif ((u"Leçon" in name and "Menu" not in name) or u"Exercice" in name ):
	...
	def event_foreground(selwf, obj, nextHandler):
		printDebug  ("event_foreground name = \"%s\", childCount = %s" % (obj.name, obj.childCount))
		...
		voiceControl.updateSettings(obj)
	...
	def event_gainFocus(selwf, obj, nextHandler):
		...
		if not GB_moduleHasFocus:
			...
			return
		# pour ne par dire le titre de la fenetre (lesson 1B
		#if  obj.windowClassName == "ThunderRTForm":
			printDebug ("out event_gainfocus appModule")
			#return
			
		#printDebug("nextHandler")		
		nextHandler()
		printDebug ("out event_gainfocus appModule")
